...
Route::middleware(['api', 'throttle:500,1'])->get('/categorys/{placeId}', function($placeId){
    return \Illuminate\Support\Facades\DB::select("SELECT id, category_name FROM categorys WHERE (SELECT COUNT(*) FROM products WHERE products.place_id = $placeId AND products.category_id = categorys.id) > 0");
});
